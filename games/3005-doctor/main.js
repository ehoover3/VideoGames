...
Game.prototype = {
  ...
  hasBodyByType: function (type) {
    return Boolean(
      _.find(this.bodies, function (body) {
        return body instanceof type;
      })
    );
  },
...
var getCaveProperties = function (game) {
  ...
  var _6_6 = {
    ...
    bodies: [new CaveFire(game, { x: 80, y: 72 }), new CaveFire(game, { x: 176, y: 72 }), new Merchant(game, { x: 128, y: 72 }), new ItemPrice(game, { x: 45, y: 115 }), new MagicShield(game, 160, { x: 80, y: 100 }), new KeyItem(game, 100, { x: 128, y: 100 }), new BlueCandle(game, 60, { x: 176, y: 100 })],
  };

  var _7_6 = {
    text: "IT'S A SECRET\nTO EVERYBODY.",
    bodies: [new CaveFire(game, { x: 80, y: 72 }), new CaveFire(game, { x: 176, y: 72 }), new CaveMoblin(game, { x: 128, y: 72 }), new CaveRupee(game, { x: 128, y: 100 }, 100)],
  };
  ...
  if (player.hasSword) {
    _7_7 = {
      text: "",
      bodies: [new CaveFire(game, { x: 80, y: 72 }), new CaveFire(game, { x: 176, y: 72 })],
    };
  } else {
    _7_7 = {
      text: "IT'S DANGEROUS TO GO\nALONE! TAKE THIS.",
      bodies: [new CaveFire(game, { x: 80, y: 72 }), new CaveFire(game, { x: 176, y: 72 }), new OldMan(game, { x: 128, y: 72 }), new Sword(game, { x: 128, y: 100 })],
    };
  }
...
var getEnemyLocations = function (game) {
  ...
  var _7_6 = {
    bodies: [new CaveEntrance(game, { x: 120, y: 24 })],
  };

  var _8_7 = {
    bodies: [new RedOctorok(game, "up", { x: 200, y: 100 }), new RedOctorok(game, "down", { x: 86, y: 40 }), new RedOctorok(game, "down", { x: 86, y: 134 })],
  };
...
var createBombExplosion = function (game, center) {
  //The BombExplosion actually does the damage, not the Cloud
...
// The approach is to test for five situations.
// If true, bodies are not colliding.
// If none are true, bodies are colliding.
...
var doBodiesCollide = function (b1, b2) {
  var isColliding = !(b1 === b2 || b1.center.x + b1.size.x / 2 < b2.center.x - b2.size.x / 2 || b1.center.y + b1.size.y / 2 < b2.center.y - b2.size.y / 2 || b1.center.x - b1.size.x / 2 > b2.center.x + b2.size.x / 2 || b1.center.y - b1.size.y / 2 > b2.center.y + b2.size.y / 2);
  return isColliding;
};
